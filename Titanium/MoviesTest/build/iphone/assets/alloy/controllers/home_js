...
function Controller() {
        ...
        if (CoreMotion.isDeviceMotionAvailable() && !CoreMotion.isDeviceMotionActive()) {
            ...
            CoreMotion.startDeviceMotionUpdates(function(e) {
                if (e.success) {
                    ...
                    for (var i = 0, num_cells = cells.length; num_cells > i; i++) {
                        var cell = cells[i];
                        cell.updateViews({
                            "#image": {
                                top: imageTop,
    ...
    function unregisterForMotionUpdates() {
        CoreMotion.isDeviceMotionActive() && CoreMotion.stopDeviceMotionUpdates();
    }
    ...
    $.__views.settingsCell = Alloy.createController("views/list_static_cell", {
        id: "settingsCell",
        __parentSymbol: $.__views.lists_container
    });
    ...
    $.settingsCell.getView().addEventListener("click", function() {
